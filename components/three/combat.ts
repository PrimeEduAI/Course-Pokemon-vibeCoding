...
import { CHARIZARD, PIKACHU, hasStab, type MoveDef } from '@/lib/battle/moves'
...
/** 玩家招式命中噴火龍：傷害 + 白閃 + 擊退 + 傷害數字 */
export function hitEnemy(move: MoveDef, hitPos: Vector3) {
  const mult = getTypeMult(move.type, CHARIZARD.types)
  const dmg = computeDamage(move, PIKACHU, CHARIZARD, hasStab(move, PIKACHU), mult)
  const st = useBattle.getState()
  st.dealDamageToEnemy(dmg)
  st.addPopup({
    text: mult >= 2 ? `${dmg} ×2!` : `${dmg}`,
...
export function hitPlayer(move: MoveDef, hitPos: Vector3) {
  const mult = getTypeMult(move.type, PIKACHU.types)
  const raw = computeDamage(move, CHARIZARD, PIKACHU, hasStab(move, CHARIZARD), mult)
  const dmg = Math.max(1, Math.round(raw * ENEMY_DAMAGE_SCALE))
  const st = useBattle.getState()
