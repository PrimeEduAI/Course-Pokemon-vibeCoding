 ...
 */
export default function PokemonRenderable({ dexId, mode, facing, targetHeight, arenaGen, entity }: PokemonRenderableProps) {
  const name = spriteNameForDex(dexId)
  const artworkUrl = `/assets/artwork/${dexId}.png`

  if (mode === 'modern' || !name) {
    return <PokemonModel dexId={dexId} targetHeight={targetHeight} entity={entity} />
  }
